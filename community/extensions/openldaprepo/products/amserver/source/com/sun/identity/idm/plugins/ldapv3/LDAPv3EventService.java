 ...
 *
 * $Id: LDAPv3EventService.java,v 1.1 2009/08/10 17:35:43 superpat7 Exp $
 *
 */
...
import com.sun.identity.shared.ldap.LDAPIntermediateResponse;
import com.sun.identity.shared.ldap.LDAPSyncInfoMessage;
...
import com.sun.identity.shared.ldap.controls.LDAPSyncRequestControl;
import com.sun.identity.shared.ldap.controls.LDAPSyncStateControl;
import com.sun.identity.shared.ldap.controls.LDAPSyncDoneControl;
...
public class LDAPv3EventService implements Runnable {
    ...
    // The replica id's that are currently in use for ldap
    // content sync requests  
    private static final Set usedReplicaIds = new HashSet();
    ...
    // RFC4533 - LDAP Content Synchronization Operation
    private boolean rfc4533Supported = false;
    ...
    public synchronized String addListener(SSOToken token,
        ...
        if (!pSearchSupported && !adNotificationSupported &&
                !rfc4533Supported) {
            debugger.error("LDAPv3EventService.addListener: unable to " +
                    "determine if psearch, notification or content sync " +
                    "is supported. randomID=" + randomID);
            Object[] args = { CLASS_NAME };
            throw new IdRepoException(IdRepoBundle.BUNDLE_NAME, "218", args);
        }
        LDAPConnection lc = null;
        ...
        } else if (adNotificationSupported) {
        ...
        } else if (rfc4533Supported) {
            LDAPSyncRequestControl syncReqCtrl;

            replicaId = this.getNextReplicaId();
            final byte [] cookie = this.generateCookie( replicaId ).getBytes();

            syncReqCtrl = new LDAPSyncRequestControl(
                  LDAPSyncRequestControl.MODE_REFRESH_AND_PERSIST,
                  cookie, false );
            cons.setServerControls(syncReqCtrl);
            attrs = new String[] { "objectclass" };
        } else {
            if (debugger.messageEnabled()) {
                debugger.message(
                      "LDAPv3EventService." +
                      "processSearchResultMessage() - no extended search " +
                      "capability supported.");
            }
            throw new IllegalStateException(
                  "No extended search capability supported.");
        }
        ...
        Request request = new Request(id, reqID, token, base, scope, filter,
                ...
                pluginInstance, serverNames, psIdKey, replicaId);
    ...
    protected boolean processResponse(LDAPMessage message) {
        ...
        if (request == null) {
            ...
            final LDAPIntermediateResponse rsp =
                    (LDAPIntermediateResponse) message;
            processIntermediateResponse(rsp, request);
        }
        return successState;
    }
    ...
    protected void removeListener(Request request) {
        ...
        if (connection != null) {
            ...
            try {
                if (isUsingRfc4533()) {
                    // We need to remove the replicaId from the list of
                    // used replicaIds so that it is available for future
                    // searches
                    this.returnReplicaIdToPool( request.getReplicaId() );
                }
    ...
    /**
     * On network error, create ExceptionEvent and deliver it to all listeners
     * on all events.
     */    
    ...
    protected void processSearchResultMessage(LDAPSearchResult res,
        ...
        if (pSearchSupported) {
           ...
           if (ctrls == null) {
              ...
              // Multiple controls might be in the message
              for (int i = 0; i < ctrls.length; i++) {
                 LDAPSyncStateControl syncStateCtrl = null;

                 if (ctrls[i] instanceof LDAPSyncStateControl) {
                    syncStateCtrl = (LDAPSyncStateControl) ctrls[i];
                    if (debugger.messageEnabled()) {
                       debugger.message( "LDAPv3EventService." +
                             "processSearchResultMessage() changeCtrl = " +
                             syncStateCtrl.toString() +
                             " randomID=" + randomID);
                    }

                    try {
                       // The OpenLDAP content sync feature assigns dn's with
                       // a universal id as a means of tracking moved DN's.
                       // Because OpenSSO is not designed to support universal
                       // id's, it is not capable of tracking DN moves. Rather
                       // than modify OpenSSO to provide this support at this
                       // time, it is sufficient to just specify that all
                       // objects are changed - resulting in a full reload of
                       // the cache. In the future is performance issueses
                       // arise this support could be added.  
    ...
    /**
     * Process messages of type LDAPIntermediateResponse.
     * @param message the message
     * @param request the request
     */
    protected void processIntermediateResponse(
          final LDAPIntermediateResponse message, final Request request) {
        if (debugger.messageEnabled()) {
            debugger.message(
                  "LDAPv3EventService.processIntermediateResponse()");
        }
        
        if (message instanceof LDAPSyncInfoMessage) {
            // An LDAPSyncInfoMessage is received to indicate completion
            // of initial content synchronization. Requires no action.
            //
            if (debugger.messageEnabled()) {
                debugger.message(
                      "LDAPv3EventService.processIntermediateResponse() - " +
                      "LDAPSyncInfoMessage received.");
            }
        } else {
            if (debugger.messageEnabled()) {
                debugger.message(
                      "LDAPv3EventService.processIntermediateResponse() - " +
                      "Unexpected LDAPIntermediateResponse message received.");
            }
        }
    }
        ...
        while (res.hasMoreElements()) {
            ...
            while (enumAttrs.hasMoreElements()) {
                ...
                while (enumVals.hasMoreElements()) {
                    ...
                    } else if (aVal.equals( LDAPSyncRequestControl.OID )) {
                        rfc4533Supported = true;
                        // Register the OpenLdap Content Sync controls
                        LDAPControl.register( LDAPSyncStateControl.OID,
                              LDAPSyncStateControl.class );
                        LDAPControl.register( LDAPSyncDoneControl.OID,
                              LDAPSyncDoneControl.class );
                    }
                } // inner most while
            } // inside while
        } // outer most while.
    ...
    /*
     * Is RFC4533 being used as the extended search mechanism.
     */
    private boolean isUsingRfc4533() {
        return (!pSearchSupported && !adNotificationSupported &&
              rfc4533Supported);
    }
    
    /*
     * Generate a cookie value to be used in the content sync request.
     * The cookie includes the specified replicaId.   
     */
    private String generateCookie( final int replicaId ) {
        final String cookie = "rid=" + Integer.toString( replicaId );
        return cookie;
    }

    /*
     * Gets the next available replicaId to be used in generating an ldap
     * content sync request.
     */
    private synchronized int getNextReplicaId() {
        final int maxReplicaId = 999;
        final int minReplicaId = 500;
        int replicaId;

        for ( replicaId = maxReplicaId; replicaId >= minReplicaId;
              replicaId-- ) {
           if ( !usedReplicaIds.contains( new Integer(replicaId) ) ) {
               usedReplicaIds.add( new Integer(replicaId) );
               break;
           }
       }
    ...
    /*
     * Returns the replicaId to the pool of unused replicaId's.
     * If the replicaId is not currently in use, this method does nothing.
     */
    private synchronized void returnReplicaIdToPool( final int replicaId ) {
        usedReplicaIds.remove( new Integer( replicaId ) );
    }
