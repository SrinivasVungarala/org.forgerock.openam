 ...
 *
 * $Id: PassphraseResetServlet.java,v 1.2 2008/06/25 05:43:42 qcheng Exp $
 *
...
import org.forgerock.openam.authentication.modules.passphrase.ui.PPResetInvalidURLViewBean;
import org.forgerock.openam.authentication.modules.passphrase.ui.PPResetUncaughtExceptionViewBean;
...
/**
 * <code>PassphraseResetServlet</code> is the controller Servlet for the Passphrase Reset.
 */
public class PassphraseResetServlet extends ApplicationServletBase implements Constants {
	/**
	 * Default module URL
	 */
	public static final String DEFAULT_MODULE_URL = "../ppui";

	// All module servlets use this debug object.
	private static Debug debug = Debug.getInstance("amPassphrase");
	
	private static final String URL_LOCALE = "locale";

	/*
	 * The the default webContainer iWS can use hidden form variable gx_charset
	 * to detect the charset encoding of POST data. However it is an iws
	 * specific feature and not supported by BEA & IBM. In BEA and IBM
	 * webcontainers we use setPageEncoding() API to set the encoding correctly.
	 * This boolean variable is set to TRUE if the web container is BEA or IBM
	 */
	...
	static {
		...
		if (webContainer != null) {
			/*
			 * If webcontainer name starts with BEA , we assume that we are
			 * using BEA weblogic appserver 6.1 onwards
			 */
			setRequestEncoding = (webContainer.indexOf("BEA") == 0);
		}
		if (debug.messageEnabled()) {
			debug.message("Webcontainer=[" + webContainer + "]setRequestEncoding=" + setRequestEncoding);
		}
	}
	 ...
	 * Package name
	 */
	public static String PACKAGE_NAME = getPackageName(PassphraseResetServlet.class.getName());
	 ...
	 */
	public PassphraseResetServlet() {
		super();
	}

	/**
	 * Using this callback the character set to be used for decoding POST/GET
	 * data will be set. This module is applicable to only those webcontainers
	 * which does not support gx_charset field for parameter encoding which is
	 * BEA. Servlet 2.3 API setCharacterEncoding() is used to set the character
	 * set value.
	 * 
	...
	/**
	 * Forwards to invalid URL view bean, in case of an invalid target request
	 * handler (page).
	 * 
	 * @param requestContext request context
	...
	protected void onRequestHandlerNotFound(RequestContext requestContext, String handlerName) throws ServletException {
		...
		ViewBean targetView = viewBeanManager.getViewBean(PPResetInvalidURLViewBean.class);
	...
	protected void onUncaughtException(RequestContext requestContext, Exception e) throws ServletException, IOException {
		PPResetModelImpl.debug.error("pasphrase.onUncaughtException", e);
		ViewBeanManager viewBeanManager = requestContext.getViewBeanManager();
		ViewBean targetView = viewBeanManager.getViewBean(PPResetUncaughtExceptionViewBean.class);
		targetView.forwardTo(requestContext);
		throw new CompleteRequestException();
	}
}