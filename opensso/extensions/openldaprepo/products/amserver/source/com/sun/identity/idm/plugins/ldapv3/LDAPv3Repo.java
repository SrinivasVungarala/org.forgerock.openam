...
public class LDAPv3Repo extends IdRepo {
    ...
    public String create(SSOToken token, IdType type, String name, Map attrMap)
        ...
        try {
            ...
            attrMap = getAttrMapWithNamingAttrNameAndValue( attrMap, eDN );
    ...
    private void setAttributes(SSOToken token, IdType type, String name,
        ...
        attributesCase = doPasswordEncode(type, name, attributesCase, 
            unicodePwd);
        attributesCase = doPasswordEncode(type, name, attributesCase, 
            userPassword);
    ...
    */
    ...
    /**
     * Returns a possibly modified copy of the specified attribute map.
     * <p>
     * This method guarantees that the returned attribute map contains the
     * naming attribute and value determined by parsing the specified
     * distinguished name (DN). If the specified map doesn't contain the naming
     * attribute and value this method adds them to the copy returned.  If the
     * specified map does contain the naming attribute, its values are replaced
     * by the value of the naming attribute from the dn parameter.  Replacing
     * the value, rather than appending the new value, is necessary to prevent
     * errors resulting from entries that have single-valued attributes as
     * naming attributes.
     * </p>
     * <p>
     * For example, if the dn parameter has the value
     * <tt>uid=jsmith,ou=People,dc=opensso,dc=java,dc=net</tt> and the 
     * attribute map contains a key of <tt>uid</tt>, but with a value of
     * <tt>johnsmith</tt>, this method will change that value to
     * <tt>jsmith</tt>.
     * </p>
     *
     * <p>
     * Note that to be legal an LDAP entry must contain the exact attribute
     * and value in its attribute set that is used in its dn as the naming
     * attribute.
     * </p>
     *
     * @param attrMap an attribute map, which can't be null
     * @param dn the distinguished name, which can't be null and must be a
     * valid LDAP DN
     *
     * @returns an attribute map guaranteed to have the naming attribute with
     * a legal value
     *
     * @throws IdRepoException if unable to create a legal attribute map
     */
    private Map getAttrMapWithNamingAttrNameAndValue( final Map attrMap,
          final String dn ) throws IdRepoException
    {
       if (null == attrMap)
       {
          throw new IllegalArgumentException( "Parameter 'attrMap' is null." );
       }
       
       if (null == dn)
       {
          throw new IllegalArgumentException( "Parameter 'dn' is null." );
       }
       
       final Map legalAttrMap = new HashMap( attrMap );
       legalAttrMap.putAll( this.getNamingAttrAttrMap( dn ) );
       
       return legalAttrMap;
    }
    
    /**
     * Gets an attribute map containing the naming attribute and value for
     * the specified distinguished name (DN).
     * <d>
     * In LDAP, the naming attribute is the leftmost attribute in the
     * distinguished name.  For example, given a DN of
     * <tt>uid=jsmith,ou=People,dc=opensso,dc=java,dc=net</tt> the naming
     * attribute is the <tt>uid</tt> attribute. If this method were passed this
     * dn, it would return a Map with a single mapping as follows:
     * <ul>
     * <li>Key - the string 'uid'</li>
     * <li>Value - a set containing a single string value of 'jsmith'</li>
     * </ul>
     *
     * @param dn the distinguished name, which can.t be null or empty and must
     * be a valid LDAP DN
     *
     * @return a map containing the naming attribute and its value
     *
     * @throws IdRepoException if the specified dn is invalid
     */
    private Map getNamingAttrAttrMap( final String dn ) throws IdRepoException
    {
       if (null == dn)
       {
          throw new IllegalArgumentException( "Parameter 'dn' is null." );
       ...
       {
          final String errorMsg = "Parameter 'dn' [" + dn + "] " +
                "is not a valid LDAP DN";
          ...
          throw new IdRepoException( errorMsg );
       }

       // The naming attribute is the last element in the list
       //
       final int indexOfNamingAttrInDn = ldapName.size() - 1;
       
       // If the DN is empty we need to throw an exception
       //
       if (indexOfNamingAttrInDn < 0)
       {
          final String errorMsg = "Parameter 'dn' [" + dn + "] contains" +
                "no relative distinguished names.";
       ...
       else
       {
          ...
          Rdn namingAttrRdn = (Rdn) rdns.get( indexOfNamingAttrInDn );
          return Collections.singletonMap( namingAttrRdn.getType(),
                Collections.singleton( (String) namingAttrRdn.getValue() ) );
       }
    }
}
