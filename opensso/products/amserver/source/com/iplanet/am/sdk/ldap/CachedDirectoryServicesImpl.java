...
public class CachedDirectoryServicesImpl extends DirectoryServicesImpl
    ...
    private static void initializeParams() {
        ...
        try {
            ...
            if (debug.messageEnabled()) {
                debug.message("CachedDirectoryServicesImpl.intializeParams() "
                        + "Caching size set to: " + maxSize);
            }
        } catch (NumberFormatException ne) {
            maxSize = 10000;
            debug.warning("CachedDirectoryServicesImpl.initializeParams() "
                    + "- invalid value for cache size specified. Setting "
                    + "to default value: " + maxSize);
        }
    }

    private CachedDirectoryServicesImpl() {
        super();
        initializeCache();
        cacheStats = CacheStats.createInstance(getClass().getName(), debug);
    }
    ...
    protected static synchronized IDirectoryServices getInstance() {
        if (instance == null) {
            debug.message("CachedDirectoryServicesImpl.getInstance(): "
                    + "Creating new Instance of CachedDirectoryServicesImpl()");
            instance = new CachedDirectoryServicesImpl();
        }
        return instance;
    }
...
//            }
        ...
        if (debug.messageEnabled()) {
            debug.message("CachedDirectoryServicesImpl.dirtyCache(): "
                    + "Cache dirtied because of Event Notification. Parameters"
                    + " - eventType: " + eventType + ", cosType: " + cosType
                    + ", aciChange: " + aciChange + ", fullDN: " + origdn
                    + "; rfcDN =" + dn);
        }
    }
    ...
    // ***********************************************************************
    // Overriden Methods of DirectoryServicesImpl class below
    // ***********************************************************************
    public void createEntry(SSOToken token, String entryName, int objectType,
        ...
        String cacheDN = NamingAttributeManager.getNamingAttribute(objectType)
                + "=" + entryName + "," + parentDN;
    ...
    private void validateEntry(SSOToken token, CacheBlock cb)
        ...
        if (!cb.hasExpiredAndUpdated() && !cb.isExists()) {
            ...
            if (debug.messageEnabled()) {
                debug.message("CachedDirectoryServicesImpl.validateEntry():"
                        + " DN " + dn + " got from DS & exists: " + isPresent);
            }
            if (isPresent) {
                ...
                throw new AMException(AMSDKBundle
                        .getString("461", args, locale), "461", args);
            }
        }
    }
    ...
    public boolean doesEntryExists(SSOToken token, String entryDN) {
        ...
        if (cb != null && !cb.hasExpiredAndUpdated()) {
            if (debug.messageEnabled()) {
                debug.message("CachedDirectoryServicesImpl.doesEntryExist(): "
                        + "entryDN: " + entryDN + " found in cache & exists: "
                        + cb.isExists());
            }
            return cb.isExists();
        } else {
            ...
            if (debug.messageEnabled()) {
                debug.message("CachedDirectoryServicesImpl.doesEntryExist():"
                        + " entryDN: " + entryDN + " got from DS & exists: "
                        + isPresent);
            }
    ...
    private void setOrganizationDNs(String organizationDN, Set childDNSet) {
        ...
        if (debug.messageEnabled() && !childDNSet.isEmpty()) {
            debug.message("CachedDirectoryServicesImpl.setOrganizationDNs(): "
                    + "Set org DNs as: " + organizationDN + " for children: "
                    + childDNSet);
        }
    }
    ...
    public Map getExternalAttributes(SSOToken token, String entryDN,
        ...
        if (callBackHelper.isExternalGetAttributesEnabled(orgDN)) {
    ...
    public Map getAttributes(SSOToken token, String entryDN,
        ...
        if (debug.messageEnabled()) {
            debug.message("In CachedDirectoryServicesImpl.getAttributes("
                    ...
                    + principalDN + ", " + entryDN + ", " + ignoreCompliance
                    + " method.");
        }
        ...
        if (cb != null) {
            ...
            if (debug.messageEnabled()) {
                debug.message("CachedDirectoryServicesImpl.getAttributes(): "
        ...
        if (extAttributes != null && !extAttributes.isEmpty()) {
            ...
            if (debug.messageEnabled()) {
                debug.message("CachedDirectoryServicesImpl.getAttributes(): "
                        + "External attributes present. Adding them with "
                        + "original list");
            }
        ...
        if (extAttributes != null && !extAttributes.isEmpty()) {
            ...
            if (debug.messageEnabled()) {
                debug.message("CachedDirectoryServicesImpl."
                        ...
                        + " list. External Attributes: " + extAttrNames);
            }
    ...
    public Map getAttributes(SSOToken token, String entryDN, Set attrNames,
        ...
        if (dcTreeImpl.isRequired()) { // TODO: This needs to be fixed!
        ...
        if (debug.messageEnabled()) {
            debug.message("In CachedDirectoryServicesImpl.getAttributes("
                    ...
                    + "byteValues) " + "(" + principalDN + ", " + entryDN
                    + ", " + attrNames + ", " + ignoreCompliance + ", "
                    + byteValues + " method.");
        }
        ...
        if (cb == null) { // Entry not present in cache
            if (debug.messageEnabled()) {
                debug.message("CachedDirectoryServicesImpl.getAttributes():  "
                        + "NO entry found in Cache. Getting all these " 
                        + "attributes from DS: " + attrNames);
            }
                ...
                // Check for "nsRole" and "nsRoleDN" attributes
                    ...
                    if (debug.messageEnabled()) {
                        debug.message("CachedDirectoryServicesImpl."
                                ...
                                + " attributes from DS: " + missAttrNames);
                    }
