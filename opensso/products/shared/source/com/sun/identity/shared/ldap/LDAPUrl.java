...
public class LDAPUrl implements java.io.Serializable {
    ...
    public static String decode (String URLEncoded) throws
        ...
        byte[] byteDecode;
        byteDecode = new byte[URLEncoded.length()];
        int srcPos = 0, dstPos = 0;
        try {
            while (srcPos < URLEncoded.length()) {
                if (URLEncoded.charAt (srcPos) != '%') {
                    byteDecode[dstPos++] = (byte)(URLEncoded.charAt(srcPos));
                    srcPos++;
                }
                else {
                    byteDecode[dstPos++] = (byte)((hexValue(URLEncoded.charAt (srcPos+1))<<4) |
                                        (hexValue(URLEncoded.charAt (srcPos+2))));
                    srcPos += 3;
                }
            }
        ...
        byte[] finalByteDecode;
        finalByteDecode = new byte[dstPos];
        for(int i=0; i<dstPos; i++) {
                finalByteDecode[i] = byteDecode[i];
        }

        String decoded = new String(finalByteDecode, "UTF8");
        return decoded;
    }
    ...
    public static String encode (String toEncode) throws UnsupportedEncodingException {
      // Why +10? Leave for now. Nothing seems to be broken.
        ...
        for (int currPos = 0; currPos < toEncode.length(); currPos++) {
            ...
            if ((currChar >= 'a' && currChar <= 'z') ||
              ...
              ("$-_.+!*'(),".indexOf (currChar) > 0)) {
                ...
                String tmpString = new String(String.valueOf(currChar));
                byte[] b = tmpString.getBytes("UTF8");
                for (int i = 0; i < b.length; i++) {
                    encoded.append ("%");
                    encoded.append (hexChar ((b[i] & 0xF0) >> 4));
                    encoded.append (hexChar (b[i] & 0x0F));
                }
            }
        }
      }
